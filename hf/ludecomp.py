...
def ludecomp(mat_in, mode = 'recursive', iexpr = 'False'):
    """
    Lower triangonal-Upper triangonal matrix decomposition\n
    # input requirement\n
    mat_in: squared matrix\n
    mode: 'recursive' or 'Crout', for the former, printing of expression of every element is supported, if
    set 'iexpr' to True\n
    ...
    if mode == 'recursive':
        ...
        for i in range(nline):
            for j in range(i, nline):
                ...
                term_Aij = 0
                expr_str_U_off_diag = 'U({}, {}) = A({}, {})'.format(i, j, i, j)
                for k in range(i):
                    term_Aij += L[i][k]*U[k][j]
                    word = ' - L({}, {})*U({}, {})'.format(i, k, k, j)
                    expr_str_U_off_diag += word
                mat_in[i][j] -= term_Aij
                U[i][j] = mat_in[i][j]
                if iexpr:
                    print(expr_str_U_off_diag)
