...
def matrix_plus(A, B):
    ...
    if np.ndim(A) == 2:
        ...
        print('Sorry, other dimensional matrix is not supported yet.')
        print('the first matrix you input:\n'+str(A))
        print('execute np.shape(A): '+str(np.shape(A)))
        print('the second matrix you input:\n'+str(B))
        print('execute np.shape(B): '+str(np.shape(B)))
        raise TypeError
    return result
...
def ketbra(a, b):
    ...
    for irow in range(len(a)):
        for icol in range(len(b)):
            result[irow][icol] = a[irow]*b[icol]
    return result
